...
# Setup
csv_path = r"C:\Users\franc\Downloads\linux_commands_conversations.csv"  # Sostituisci con il percorso del tuo CSV
...
# Test di performance per la creazione dell'indice FAISS
def test_faiss_index_performance():
    df = load_dataset(csv_path)
    conversations = extract_conversations(df)

    import time
    start_time = time.time()
    create_faiss_index(conversations, model)
    end_time = time.time()

    elapsed_time = end_time - start_time
    assert elapsed_time < 5, f"Creazione dell'indice FAISS troppo lenta: {elapsed_time} secondi"
